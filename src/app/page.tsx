...
function formatCommands(commands: SVGCommand[], isClosed: boolean): string {
    const body = commands.map(formatCommand).join(" ");
    return isClosed ? `${body} Z` : body;
}

function formatCommand(cmd: SVGCommand): string {
...
export default function Home() {
    ...
    const handleSetStartPoint = () => {
        if (selectedIndex !== null && currentPath && previewIndex !== null) {
            ...
            const reordered = reorderPathPreservingClosureStrict(normalized, selectedIndex);
            updatePath(previewIndex, reordered);
            setCurrentStartIndex(selectedIndex);
            setSelectedIndex(null);
            toast.success(`시작점을 #${selectedIndex}번 앵커로 설정했어요`);
        }
    };
